...
def write_outputs(running_config: RheoscaleConfig, position_df: pd.DataFrame):
    out_dir = rf'{running_config.output_dir}\{running_config.output_folder_name_prefix}_Rheoscale'
    ...
    if running_config.output_histogram_plots:
        ...
        plot_output = rf'{out_dir}\{running_config.output_folder_name_prefix}_plots'
        os.makedirs(plot_output, exist_ok=True)
        plot_all_positions(position_list, hist_list, running_config.dead_extremum, running_config.WT_val,plot_output, running_config.output_folder_name_prefix)
